...
#include <string>                    // библиотека С++ работы со строками

using namespace std;                 // работа в пространстве имён std
using CodeType = uint16_t;           // тип, используемый для хранения и извлечения кодов
              ...
              FILE * FilenameOUT)                  // указатель на файл, куда будет записываться считанная информация
{
    ...
    const char *txt_ext = ".txt";                  // объявление переменной txt_ext типа "константный указатель на символ"
...
/*----------------------------------------------------------------------*/
/*    Функция поиска в векторе     */
/*---------------------------------*/

int find(vector<string> book,                      // вектор словаря
         string ch)                                // строка для поиска в словаре
{
    // проходим по вектору
	for (int i = 0; i < book.size(); i++)
	{
        // сравниваем строку для поиска с каждым элементом словаря
		if (ch == book[i])
		{ 
			return i;                               // возвращаем индекс вектора словаря, если строка в нём найдена
		}// if(ch == book[i])
	}// for i

	return -1;                                      // возвращаем -1, если строка не найдена в векторе словаря
}



/*----------------------------------------------------------------------*/
/*    Алгоритм LZW для сжатия данных     */
/*---------------------------------------*/

void LZW (const char* FNAME,  // путь до файла архива
          FILE *file)         // указатель на файл архива, где будет произведено сжатие
{
    string input_str = "";    // строка, в которую помещаются все входные символы
	vector<string> book;      // словарь, у которого ключом является строка. Будет содержать комбинации символов
	vector<CodeType> output;  // выходной вектор
    char buffer[1];           // буфер для считывания данных из файла

    while(!feof(file))        // считывание данных из файла побитово в буфер, пока не достигнут конец файла
    {
        fread(buffer,1,1,file);
        // добавляем считанный символ в итоговую строку, содержащую все считанные символы из файла
        input_str = input_str + buffer[0];
    }// while()

    // заполнение словаря единичными символами. Формируем стандартный набор из 256 символов
	for (int i = 0; i <= 255; i++)
	{
		string ch = "";
		ch = ch +  char(i);   // формируем символ словаря
		book.push_back(ch);   // добавляем символ в конец словаря
	}// for

	string s = "";            // пустая цепочка
	string c = "";            // строка для взятия очередного символа строки
	
    int i = 0;

    // проход по всем символам сформированной строки архива
	while (i < input_str.size())
	{
		c = input_str[i];     // взяли очередной символ строки

        // если цепочка s+c есть в словаре, то добавляем к цепочке s символ c - увеличиваем цепочку
        // проверка, встречалась ли данная комбинация символов в предыдущих итерациях алгоритма
		if (find(book, s + c) != -1)
		{
            // если цепочка s+c найдена в словаре, это означает, что она была добавлена в словарь
            // ранее, и мы можем продолжать расширять текущую цепочку символов s путем добавления
            // символа c к ней в надежде заменить большее число символов
			s = s + c;
		}

        // если цепочки символов s + c нет в словаре, то добавляется код для цепочки s в выходной
        // вектор output, добавляется новая цепочка s + c в словарь и цепочка s обновляется до c
		else
		{
			output.push_back(find(book, s)); // вывод кода для заданной цепочки s
			book.push_back(s + c);           // добавляем в словарь новую цепочку символов
			s = c;                           // теперь цепочка будет начинаться с того симвого, с добавлением которой
		                                     // у нас появилась цепочка, которой раньше не было в словаре
		}// if (find(book, s + c) != -1)

		i = i + 1;                           // продвигаемся по строчке
	}// while()

	output.push_back(find(book, s));         // вывод кода для оставшийся цепочки
    file = freopen(FNAME,"wb",file);         // повторное открытие файла для его очистки и дальнейшей записи данных
     
    // после сжатия всех данных, происходит перезапись данных в тот же файл с использованием
    // функции fwrite. Записываются двухбайтовые коды, представляющие числовые значения из выходного
    // вектора output
    for(int i = 0;i<output.size();i++)
    {
        fwrite(&output[i],2,1, file);        // записываем данные
    }// for i
...
void archiver()
{
    ...
    // закрываем файл, чтобы данные сохранились перед записью
    fclose(rest_arch);
    // открываем файл для сжатия после архивации
    rest_arch = fopen(info_arch[3], "rb+");
    // происводим сжатие файла алгоритмом LZW
    LZW(info_arch[3], rest_arch);

    // размер файла после архивации
    // перемещаем указатель файла в конец
    fseek(rest_arch, 0, SEEK_END);
    // получаем текущую позицию указателя файла (которая является размером файла)
    int size = ftell(rest_arch);
    printf(" Размер архива после сжатия равен %i байт.\n", size);
...
/*----------------------------------------------------------------------*/
/*   Алгоритм LZW для раскодирования файлов   */
/*--------------------------------------------*/

void decoding_LZW(const char* FNAME,                   // путь к файлу архива
                  FILE *file)                          // указатель на файл архива, где будет произведено раскодирование
{
	vector<string> book;                               // словарь, у которого ключом является строка. Будет содержать комбинации символов
	string output = "";                                // выходная строка
	vector<int> code;                                  // вектор кодов, считанных из файла архива

	uint16_t buf = 0;                                  // беззнаковый целочисленный тип с фиксированной длиной в 16 бит

	
    while (!feof(file))                                // считывание данных из файла побитово в буфер, пока не достигнут конец файла
    {
        fread(&buf, 2, 1, file);                       // читаем в буфер по 2 байта, т.к. буфер занимает 16 бит
        code.push_back(buf);                           // добавляем считанный символ в итоговую строку, содержащую все считанные коды из файла
	}// while()

    // заполнение словаря единичными символами. Создание того же начального словаря, что и при кодировании. В данном случае 256 символов ASCII
	for (int i = 0; i <= 255; i++)
	{
		string ch = "";                                // создаём пустую строку
		ch = ch + char(i);                             // добавляем символ ASCII
		book.push_back(ch);                            // добавляем символ в словарь
	}// for i


	int i = 0;                                         // вспомогательная переменная для цикла
	int k = 0;                                         // считанный из строки код

	int old = -1;                                      // код предыдущей расскодированной строки
	string s = "";                                     // вспомогательная строка для формирования словаря

    // проходим по вектору символов закодированного архива
	while (i < code.size())                            // пока есть коды в последовательности
	{
		k = code[i];                                   // считываем код - код является индексом в массиве словаря

        // если данный код присутствует в словаре
		if (k <= book.size() - 1)
		{
			output = output + book[k];                 // код присутствует в словаре, его соответствующая строка добавляется к выходной строке output
			
            // old не равно -1 (то есть это не первый символ)
            if (old != -1)
			{
                // добавляем в словарь строку, которая состоит из предыдущий расскодированной строки и первого символа новой строки
				book.push_back(book[old] + book[k][0]); 
			}// if(old != -1)
			old = k;                                   // устанавливаем old, равным текущему коду k
		}

        // в словаре нет строки для кода code[i]
		else                                        
		{
			s = book[old] + book[old][0];              // создается новая строка s, состоящая из предыдущей раскодированной строки и первого символа предыдущей строки
			output = output + s;                       // строка добавляется к выходной строке output
			book.push_back(s);                         // строка добавляется в словарь book
			old = k;                                   // устанавливаем old, равным текущему коду k
		}// if(k <= book.size() - 1)

		i = i +1; // проход по всем кодам
	}// while()


    // повторное открытие файла для его очистки для дальнейшей записи данных
    ...
    {
        fwrite(&output[i],1,1, file);                  // записываем данные
    }// for i

	fclose(file);                                      // закрываем файл для записи
    return;                                            // возвращаем обещанное значение
}
...
void unarchiver()
{
    ...
    // Раскодирование файла архива для его послед. разархивации
    decoding_LZW(file_path, rest_arch);

    // открытие раскодированного файла архива
    rest_arch = fopen(file_path, "rb+");
    ...
    while(true)
    {
        ...
        // пользователь не знает пароль. Вводит выход
        else if (strcmp(password_user, "exit") == 0)
        {
            ...
            fclose(rest_arch);
            rest_arch = fopen(file_path, "rb+"); 
            LZW(file_path, rest_arch);     // сжатие данных в случае неудачи
            fclose(rest_arch);
            return;                           // возвращаем обещанное значение
        }
    ...
    while ((fgets(line, sizeof(line), rest_arch) != NULL) && (i < count_files))
    {
        ...
        if (strcmp(line, "*****\n") == 0) 
        {
            ...
            const char *txt_ext = ".txt";                          // объявление переменной txt_ext типа "указатель на символ"
