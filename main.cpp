...
int find(vector<string> book, string ch) // функция поиска
{
...
void LZW(const char* FNAME, FILE *file) // передаём название конечного файла и его файловый поток
{
    string input_str = "";    // строка, в которую помещаются все входные символы
	vector<string> book;      // словарь, у которого ключом являтеся строка
	vector<CodeType> output;  // выходной вектор
    char buffer[1];           // буфер для считвания данных из файла


    while(!feof(file)) // считывание данных из файла
    {
	...
	for (int i = 0; i <= 255; i++) // заполнение словаря единичными символами
	{
		string ch = "";
		ch = ch +  char(i);
		book.push_back(ch);
	...
	string c = ""; // строка для взятия очередного символа
	int i = 0;

	while (i < input_str.size()) // проход по всем символам
	{
		c = input_str[i]; // взяли очередной символ

		if (find(book, s + c) != -1) // если цепочка s+c есть в словаре, то добавляем к цепочке s символ c
		{
			s = s + c;
		}

		else  // если цепочки символов не оказалось в словаре
		{
    ...
    // Производим перезапись данных в текущий файл
...
void decoding_LZW(const char* FNAME, FILE *file)
{
	vector<string> book;
	string output = "";
	vector<int> code;
	...
	{
		k = code[i]; // считываем код 

		if (k <= book.size() - 1) // если данный код присутствует в словаре
		{
			output = output + book[k];
			if (old != -1)
			{
				book.push_back(book[old] + book[k][0]);  // добавляем в словарь строку, которая состоит из предыдущий расскодированной строки и первого символа новой строки
			}
		...
		else // в словаре нет строки для кода code[i]
		{
			s = book[old] + book[old][0];
			output = output + s;
			book.push_back(s);
			old = k;
		}

		i++; // проход по всем кодам
	}


	file = freopen(FNAME,"wb",file);  // повторное открытие файла для его очистки для дальнейшей записи данных
...
void unarchiver()
{
    ...
    FILE *rest_arch = fopen(file_path, "rb+");  // открытие файла архива для его раскодирования
    ...
    rest_arch = fopen(file_path, "rb+");  // открытие раскодированного файла архива
    ...
    while(true)
    {
        ...
        else if (strcmp(password_user, "exit") == 0)
        {
            ...
            fclose(rest_arch); // закрытие файла для перевода потока чтения и записи в начало файла( у меня почему-то функция fseek не фурычит)
